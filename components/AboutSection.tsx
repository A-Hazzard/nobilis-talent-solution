        ...
        <div className="border-2 border-primary/20 rounded-3xl p-6 lg:p-8 bg-background/50 backdrop-blur-sm">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            ...
            <div className="relative" data-animate>
              ...
              <div className="absolute -bottom-24 -left-6 animate-float hidden sm:block" style={{ animationDelay: '2s' }}>
              ...
              <div className="mb-8">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                  {achievements.map((achievement, index) => (
                    <div key={index} className="flex flex-col items-center gap-2 sm:gap-3 rounded-xl border bg-card p-3 sm:p-4 text-card-foreground hover:shadow-md transition-shadow duration-200">
                      <CheckCircle className="h-6 w-6 text-primary mb-2" />
                      <p className="text-xs sm:text-sm leading-relaxed">{achievement}</p>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <ul className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  {credentials.map((credential, index) => (
                    <li key={index} className="flex items-start gap-2 text-sm">
                      ...
                      <span className="text-xs sm:text-sm leading-relaxed">{credential}</span>
                    </li>
                  ))}
                </ul>
              </div>
